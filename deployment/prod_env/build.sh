...
# ==============================================================================
# MinerU PDF 解析服务 - 生产环境一键脚本
# ==============================================================================
# 核心设计：.env.prod 已配好 → 直接 ./build.sh 即可启动容器
# ==============================================================================
...
container_running() {
    docker ps --format "{{.Names}}" | grep -q "^mineru-api-prod$"
}
...
show_access() {
    ...
    echo ""
    echo -e "  ${YELLOW}生产环境特性:${NC}"
    echo -e "    - LOG_LEVEL=WARNING，最小化日志输出"
    echo -e "    - 访问日志已关闭，API 文档已关闭"
    echo -e "    - restart=always，异常自动恢复"
    echo -e "    - 并发请求数=8，渲染线程数=4，OMP=8"
    echo -e "    - 内存限制 16G，CPU 限制 8 核"
    echo -e "    - 任务保留 48h，日志上限 200m×10"
    echo ""
    echo -e "${GREEN}============================================${NC}"
}

show_help() {
    echo "MinerU PDF 解析服务 - 生产环境部署脚本"
    echo ""
    ...
    echo "  logs          查看实时日志"
    echo "  status        查看容器状态"
    echo "  export        导出镜像为 tar"
    echo "  load <tar>    从 tar 加载镜像"
    echo ""
}
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        build|start|stop|restart|rebuild|logs|status|export) CMD="$1"; shift ;;
        load)               CMD="$1"; shift; EXTRA_ARG="${1:-}"; [ -n "$EXTRA_ARG" ] && shift ;;
...
do_export() {
    local filename="${EXPORT_DIR}/$(echo ${IMAGE_TAG} | tr '/:' '_').$(date +%Y%m%d_%H%M%S).tar"
    mkdir -p "${EXPORT_DIR}"
    print_info "导出镜像 ${IMAGE_TAG} → ${filename} ..."
    docker save -o "${filename}" "${IMAGE_TAG}"
    print_ok "导出完成"; ls -lh "${filename}"
}

do_load() {
    local filepath="${1}"
    if [ -z "${filepath}" ] || [ ! -f "${filepath}" ]; then
        print_err "请指定 tar 文件: ./build.sh load <xxx.tar>"
        exit 1
    fi
    print_info "加载镜像: ${filepath} ..."
