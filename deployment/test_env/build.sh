...
# ==============================================================================
# MinerU PDF 解析服务 - 测试环境一键脚本
# ==============================================================================
# 核心设计：.env.test 已配好 → 直接 ./build.sh 即可启动容器
# ==============================================================================
...
container_running() {
    docker ps --format "{{.Names}}" | grep -q "^mineru-api-test$"
}
...
show_access() {
    echo ""
    echo -e "${GREEN}============================================${NC}"
    echo -e "${GREEN}   MinerU PDF 解析服务测试环境已启动${NC}"
    echo -e "${GREEN}============================================${NC}"
    echo ""
    ...
    echo ""
    echo -e "  ${YELLOW}测试环境特性:${NC}"
    echo -e "    - LOG_LEVEL=INFO，不暴露 DEBUG 详细日志"
    echo -e "    - 不挂载 mineru/，代码稳定不可热重载"
    echo -e "    - restart=always，异常自动恢复"
    echo -e "    - 并发请求数=6，渲染线程数=4，内存限制 8G"
    echo -e "    - 日志上限 100m×5"
    echo ""
    echo -e "${GREEN}============================================${NC}"
}
